...
library(here)
library(jsonlite)
source(here::here('R', 'search_filter.R'))
source(here::here('R', 'prepare_data.R'))
source(here::here('R', 'create_reatable.R'))
```
...
tbl_data <- bccc |>
  ...
  filter(
    VACANCY_LAST_UPDATE >= dataset_last_update - 1 -3
  ) 
...
div(
  class = "filters",
  search_filter("filter_name", "Search by Facility Name", shared_data, "title"),
  select_filter("filter_genres", "Genre", shared_data, "genres"),
  range_filter("filter_score", "Min Score", shared_data, "score", suffix = "%")
)
```
